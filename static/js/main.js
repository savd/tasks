...
(function() {
  var delegates_add, delegates_del, delegates_update, doLogout, init, tasks_add, tasks_cancel_edit, tasks_del, tasks_edit, tasks_edit_save, tasks_strike, tasks_update;
  ...
  init = function() {
    ...
    $$('div#header > span.a.logout')[0].on('click', function() {
      return doLogout();
    });
    tasks_update();
    delegates_update();
  ...
  tasks_update = function() {
    return Xhr.load('/tasks/show', {
      onSuccess: function() {
        ...
        $('tasks').find('td.task_body').forEach(function(el) {
          return el.stopObserving('dblClick').on('dblclick', function(e) {
            return tasks_edit(e);
          });
        });
        return $('tasks').find('span.strike').forEach(function(el) {
          return el.stopObserving('click').on('click', function(e) {
            return tasks_strike(e);
          });
        });
      }
    });
  ...
  tasks_strike = function(e) {
    e.preventDefault();
    return $('tasks').find("td[rel='" + (e.target.get("rel")) + "']")[0].setStyle("text-decoration: line-through");
  };
